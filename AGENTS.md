...
## プロジェクト固有(hyakunin-lens)

- **正体**: AI研究者・エージェント実装者 100 名の発信ダッシュボード(百人レンズ)。データは 2026-08-20 収集のスナップショット(746 件)で、自動更新はスコープ外
- **構成**: `data/people.json`(100 名)+ `data/meta.json`(語彙・収集日)→ `src/build.py` + `src/template.html` → `out/index.html`(自己完結・単一 HTML)
